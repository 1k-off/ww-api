...
func (r *repository) Delete(url string) {
	_, _ = r.uptimeCollection.DeleteMany(context.Background(), bson.M{"metadata.url": url})
	_, _ = r.sslCollection.DeleteMany(context.Background(), bson.M{"metadata.url": url})
	_, _ = r.domainExpirationCollection.DeleteMany(context.Background(), bson.M{"metadata.url": url})
}
